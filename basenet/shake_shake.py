...
class Shake_Shake(nn.Module):
    def __init__(self, num_classes):
        ...
        input_shape_sen1 = (1, 8, 32, 32)
        input_shape_sen2 = (1, 10, 32, 32)

        n_classes = num_classes

        base_channels = 128 #config['base_channels']
        depth = 26#config['depth']
        ...
        self.conv_sen1 = nn.Conv2d(
            ...
            padding=1,
            bias=False)
        self.bn_sen1 = nn.BatchNorm2d(base_channels/2)

        self.conv_sen2 = nn.Conv2d(
            input_shape_sen2[1],
            n_channels[0]/2,
            kernel_size=3,
            stride=1,
            padding=1,
            bias=False)
        self.bn_sen2 = nn.BatchNorm2d(base_channels/2)
        ...
        with torch.no_grad():
            self.feature_size = self._forward_conv(
                torch.zeros(*input_shape_sen1), torch.zeros(*input_shape_sen2)).view(-1).shape[0]
    ...
    def _forward_conv(self, x_sen1, x_sen2):
        x_sen1 = F.relu(self.bn_sen1(self.conv_sen1(x_sen1)), inplace=True)
        x_sen2 = F.relu(self.bn_sen2(self.conv_sen2(x_sen2)), inplace=True)
    ...
    def forward(self, x_sen1, x_sen2):
        x = self._forward_conv(x_sen1, x_sen2)
        x = x.view(x.size(0), -1)
        x = self.fc(x)
        return x
